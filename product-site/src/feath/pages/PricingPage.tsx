...
function PricingHUD({
  ...
  return (
    <svg viewBox="0 0 900 560" width="100%" aria-hidden="true">
      <defs>
      ...
      {Array.from({ length: 10 }, (_, row) =>
        Array.from({ length: 18 }, (_, col) => (
          <circle key={`g${row}-${col}`} cx={col * 52 + 8} cy={row * 56 + 12} r={1} fill={`rgba(${pc},0.05)`} />
        )),
      )}

      {["M 44,18 L 18,18 L 18,44", "M 856,18 L 882,18 L 882,44", "M 44,542 L 18,542 L 18,516", "M 856,542 L 882,542 L 882,516"].map(
        (d, i) => (
      ...
      {[
        ...
        <circle key={`ct${i}`} cx={x} cy={y} r={1.5} fill={`rgba(${pc},0.35)`} />
      ))}

      <circle cx={HUB.x} cy={HUB.y} r={220} fill="url(#ph-radial)" />

      <g clipPath="url(#ph-clip)">
        <rect x={0} width={900} height={120} fill="url(#ph-scan)">
          <animate attributeName="y" values="-120;680;-120" dur="12s" repeatCount="indefinite" calcMode="linear" />
        </rect>
      </g>
      ...
      <g>
        ...
        />
        <circle cx={HUB.x} cy={HUB.y} r={148} fill="none" stroke={`rgba(${pc},0.07)`} strokeWidth={1} strokeDasharray="14 10" />
      </g>
      <g>
        <animateTransform
          attributeName="transform"
          ...
          repeatCount="indefinite"
        />
        <circle cx={HUB.x} cy={HUB.y} r={104} fill="none" stroke={`rgba(${pc},0.06)`} strokeWidth={1} strokeDasharray="6 14" />
      </g>
      ...
      {mods.map((m) => {
        ...
        return (
          <g key={m.id}>
            ...
            >
              <path d={pathD(m.conn)} fill="none" stroke="transparent" strokeWidth={28} />
              <circle cx={m.nodes[0].x} cy={m.nodes[0].y} r={m.nodes[0].r + 18} fill="transparent" />
            </g>
            ...
            {isH && (
              <g style={{ animation: "fadeUp 0.18s ease forwards", pointerEvents: "none" }}>
                ...
                />
                <path
                  d={`M ${m.tipX + 6},${m.tipY} L ${m.tipX},${m.tipY} L ${m.tipX},${m.tipY + 6}`}
                  fill="none"
                  stroke={`rgba(${pc},0.7)`}
                  strokeWidth={1.2}
                />
                <path
                  d={`M ${m.tipX + 116},${m.tipY} L ${m.tipX + 122},${m.tipY} L ${m.tipX + 122},${m.tipY + 6}`}
                  fill="none"
                  stroke={`rgba(${pc},0.7)`}
                  strokeWidth={1.2}
                />
                ...
                >
                  {m.active ? "ACTIVE. CLICK TO REMOVE" : "CLICK TO ADD"}
                </text>
              </g>
            )}
          </g>
      ...
      {CORE_VERTS.map((v, i) => (
        <g key={`cv${i}`}>
          <circle cx={v.x} cy={v.y} r={16} fill={`rgba(${pc},0.06)`} stroke={`rgba(${pc},0.18)`} strokeWidth={1} />
          <circle
            cx={v.x}
            cy={v.y}
            r={8}
      ...
      <circle cx={HUB.x} cy={HUB.y} r={36} fill="none" stroke={`rgba(${pc},0.4)`} strokeWidth={1.5}>
        <animate attributeName="r" values="36;55;36" dur="4s" repeatCount="indefinite" />
        <animate attributeName="opacity" values="0.4;0;0.4" dur="4s" repeatCount="indefinite" />
      </circle>
      <circle cx={HUB.x} cy={HUB.y} r={36} fill={`rgba(${pc},0.07)`} stroke={`rgba(${pc},0.28)`} strokeWidth={1} />
      <circle
        cx={HUB.x}
        cy={HUB.y}
        r={28}
      ...
      />
      <circle cx={HUB.x} cy={HUB.y} r={19} fill={`rgba(${pc},0.35)`} />
      <line x1={HUB.x - 42} y1={HUB.y} x2={HUB.x - 20} y2={HUB.y} stroke={`rgba(${pc},0.25)`} strokeWidth={1} />
      <line x1={HUB.x + 20} y1={HUB.y} x2={HUB.x + 42} y2={HUB.y} stroke={`rgba(${pc},0.25)`} strokeWidth={1} />
      <line x1={HUB.x} y1={HUB.y - 42} x2={HUB.x} y2={HUB.y - 20} stroke={`rgba(${pc},0.25)`} strokeWidth={1} />
      <line x1={HUB.x} y1={HUB.y + 20} x2={HUB.x} y2={HUB.y + 42} stroke={`rgba(${pc},0.25)`} strokeWidth={1} />

      <g transform={`translate(${HUB.x} ${HUB.y})`} filter="url(#ph-glow)">
        <g transform="translate(-8 -13) scale(0.7)">
          <path
      ...
      <text x={450} y={534} textAnchor="middle" fontFamily="'JetBrains Mono',monospace" fontSize="7.5" letterSpacing="2" fill={`rgba(${pc},0.28)`}>
        {`NODES: ${
          1 +
          CORE_VERTS.length +
      ...
      <line x1={28} y1={530} x2={60} y2={530} stroke={`rgba(${pc},0.14)`} strokeWidth={1} />
      <text x={66} y={534} textAnchor="start" fontFamily="'JetBrains Mono',monospace" fontSize="7" letterSpacing="2.5" fill={`rgba(${pc},0.2)`}>
        FEATH SYS
      </text>
    </svg>
  );
  ...
  return (
    <div className="min-h-screen bg-background pt-20 pb-16 overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-6 mb-8 text-center">
        <Reveal>
          <span className="text-[11px] tracking-[0.22em] uppercase text-primary font-bold">Configure Your Stack</span>
          <h1
            className="text-4xl md:text-5xl font-bold text-foreground mt-2 mb-3"
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row gap-6 items-start">
          <Reveal>
            <div
              className="md:sticky md:top-24 md:w-52 shrink-0 rounded-xl border border-border bg-card/80 backdrop-blur-sm p-5 space-y-6"
              style={{ boxShadow: "0 0 0 1px rgba(61,184,112,0.05), inset 0 0 20px rgba(61,184,112,0.02)" }}
            >
              <div>
                <div className="text-[9px] font-bold tracking-[0.22em] uppercase text-primary mb-3 flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-left transition-all duration-200 ${
                        base === pkg.id
                          ...
                          : "border border-transparent hover:bg-primary/5 hover:border-border"
                      }`}
                    >
                      <div
                      ...
                      />
                      <span
                        className={`text-xs font-semibold flex-1 ${base === pkg.id ? "text-foreground" : "text-muted-foreground"}`}
                        style={{ fontFamily: "'Plus Jakarta Sans',sans-serif" }}
                      >
                        {pkg.label}
                      </span>
                      <span className="text-[10px] font-mono text-primary opacity-80">${pkg.monthly}</span>
                    </button>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-1">
                  {ADDON_LIST.map((addon) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-7 h-4 rounded-full flex items-center px-0.5 transition-all duration-200 shrink-0 ${
                        ...
                        <span
                          className={`text-xs font-semibold flex-1 ${active ? "text-foreground" : "text-muted-foreground"}`}
                          style={{ fontFamily: "'Plus Jakarta Sans',sans-serif" }}
                        >
                          {addon.label}
                        </span>
                        <span className="text-[10px] font-mono text-primary opacity-80">+${addon.monthly}</span>
                      </button>
                    );
                  })}
                </div>
              ...
              <div>
                <div className="text-[9px] font-bold tracking-[0.22em] uppercase text-muted-foreground mb-3">Investment</div>
                <div className="space-y-1 mb-4">
                  <div className="flex justify-between items-baseline">
                    <span className="text-[10px] text-muted-foreground font-mono">SETUP</span>
                  ...
                  <div className="flex justify-between items-baseline">
                    <span className="text-[10px] text-muted-foreground font-mono">MONTHLY</span>
                ...
                >
                  Book a Call <ArrowRight size={13} className="ml-1.5" />
                </GlowButton>
                <p className="text-[9px] text-muted-foreground mt-3 leading-relaxed">No contracts. SSL & hosting included.</p>
              </div>
            </div>
          </Reveal>

          <div className="flex-1 min-h-[380px] md:min-h-[500px] relative">
            <Reveal className="h-full">
              <div
                className="w-full rounded-2xl border border-border/60 bg-card/40 backdrop-blur-sm overflow-hidden"
        ...
        <Reveal delay={200}>
          <div className="flex flex-wrap items-center justify-center gap-6 mt-8 text-[10px] text-muted-foreground font-mono tracking-wide">
